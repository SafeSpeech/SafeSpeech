...
parser = argparse.ArgumentParser(description='SpeechGuard_Finetuning')
...
parser.add_argument('--protection-mode', type=str, default="NLP",
                    choices=['random_noise', 'Adv', 'SEP', 'PTA', 'NLP', 'PeNLP'], help='the protection mode of the SpeechGuard')
parser.add_argument('--checkpoints-path', type=str, default='checkpoints', help='the storing path of the checkpoints')
parser.add_argument('--noise-path', type=str, default='noises/', help='the storing path of the generated noise')
parser.add_argument('--epsilon', type=int, default=8, help='the perturbation radius boundary')
parser.add_argument('--noise-epochs', type=int, default=200, help='the iteration numbers of the noise')
...
def main():
    ...
    for epoch in range(1, training_epochs + 1):
        loss_gen_list, loss_disc = clean_train(
            epoch,
            hps,
        ...
        if epoch % 20 == 0:
            store_path = f"./{checkpoints_path}/{dataset_name}/FT/G_{model_name}_FT_{dataset_name}_{epoch}.pth"
...
def clean_train(epoch, hps, nets, optims, schedulers, scaler, loader, device):
    ...
    for batch_index, batch in enumerate(loader):
        ...
        wav_hat, l_length, attn, ids_slice, x_mask, z_mask, (z, z_p, m_p, logs_p, m_q, logs_q), \
            ...
            = net_g(
            text, text_len, spec, spec_len, speakers, tone, language, bert, ja_bert, en_bert,
        )
